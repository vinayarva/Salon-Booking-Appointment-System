async function handleSubmit(event){
    ...
    try {
        ...
        // const  result = await axios.post("http://localhost:4000/api/admin/addEmployee", user, getAuthHeaders())


        if (hidden.value === "undefined") {
            axios.post("http://localhost:4000/api/admin/addEmployee", user, getAuthHeaders())
              .then((result) => {
                console.log(result);
            ...
            axios.put(`http://localhost:4000/api/admin/updateEmployee/${ID}`, user, getAuthHeaders())
              .then((result) => {
                console.log(result);
...
function fetchEmployee() {
 ...
 axios.get("http://localhost:4000/api/admin/getEmployee", getAuthHeaders()).then((result) => {
...
function displayEmployee(data) {
        ...
        document.querySelectorAll('.edit_btn').forEach((button) => {
          button.addEventListener('click', async (event) => {
            ...
            try {
              const result = await axios.get(`http://localhost:4000/api/admin/editEmployee/${id}`, getAuthHeaders());
        ...
        document.querySelectorAll('.delete_btn').forEach((button) => {
          button.addEventListener('click', async (event) => {
            ...
            if (confirm("Are you sure you want to delete this service?")) {
              try {
                await axios.delete(`http://localhost:4000/api/admin/deleteEmployee/${id}`, getAuthHeaders());
